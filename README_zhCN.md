...
比如两个源代码文件，examples\20-source-0.js：
...
后缀 `-bc` 的为字节码文件，后缀 `-xref` 的为交叉参考文件。可以如下命令运行：
...
交叉参考文件是可选的，如果不指定，那么将无法知晓运行时错误所对应的行，比如执行：
...
`.txt` 格式的文件可以嵌入自定义c代码里，比如20-source.c：
